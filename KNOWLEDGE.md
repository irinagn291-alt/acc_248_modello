...
**Desk `signal_chain` — Studio chain weak-link**
- Home: L→R chain board + lamps.
- Invariant (unit-test this): OOS=100, overdue=80, due≤7d=50; cents/session = purchase/sessions on chains containing the gear.
- Fake that fails: Gear inventory list.

**From the shelf**
Recent Ready apps:
- Agnomen — family art_quiz — Offer-then-seat. Home is the wall label. Offer draws one free saved piece and… — architecture=Antonomasia ADT fold (Idle | Offered | Ascribed); the label is a fold over Works; Offer writes three Billets as this Work's maker, this Work's picture name, and one decoy of a Role that is Maker or Picture; Seat writes a CreditMark when the tapped Billet is this Work's string for that Role and folds Offered to Ascribed; a miss writes an InvertMark and keeps the Billets; Seat on Idle is refused; a second Offer while Offered is refused; Offer samples a Work that is not Ascribed; a crate under two Works writes Blank
